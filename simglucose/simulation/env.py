...
class T1DSimEnv(object):
    def __init__(self, patient, sensor, pump, scenario, noise=True):
...
class T1DSimEnvExtendedObs(T1DSimEnv):
    def __init__(self, patient, sensor, pump, scenario,n_samples=20, append_action=False):
       super(T1DSimEnvExtendedObs,self).__init__(patient,sensor,pump,scenario)
    ...
    def step(self, action, reward_fun=risk_diff):
        ...
        if self.append_action:
           obs = np.concatenate((np.array(self.CGM_hist[-self._n_samples:]), np.array(self.insulin_hist[-self._n_samples:], dtype=np.float64)), axis=None)
        else:
