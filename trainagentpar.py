...
from sg_rewards import stepReward3
import gym
import numpy as np
...
def make_env():
  env = TimeLimit(GymWrapper(T1DSimEnv(patient_name='adult#010',reward_fun=stepReward3,normalize=True,sequence=15,harrison_benedict=True)),370)
  return env
  ...
  with tf.compat.v2.summary.record_if(
    ...
    tf_env = tf_py_environment.TFPyEnvironment(
        parallel_py_environment.ParallelPyEnvironment(
            [lambda: make_env()] * num_parallel_environments))
    eval_tf_env = tf_py_environment.TFPyEnvironment(TimeLimit(GymWrapper(T1DSimEnv(patient_name='adult#010',reward_fun=stepReward3,normalize=True,sequence=15,harrison_benedict=True)),350))
